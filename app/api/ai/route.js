...
const MODELS = {
  ...
  fallback: "meta-llama/Meta-Llama-3.1-8B-Instruct" // Smaller model as fallback
};
    ...
    if (type === 'schedule') {
      ...
      try {
        ...
        try {
          ...
          console.error("JSON parse error with fallback model:", e);
        }
        
        return new Response(JSON.stringify({ 
          days: 7, 
          explanation: "Default schedule due to processing error." 
        }), {
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      try {
        ...
        console.error('Error with primary advice model:', primaryError);
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: `You are an AI assistant for a productivity and learning app.`
            },
            {
              role: "user",
      ...
      // General request for notes or explanations
      try {
        const completion = await openai.chat.completions.create({
          temperature: 0.7,
          model: MODELS.primary,
        ...
        console.error('Error with primary model for general request:', primaryError);
        ...
        const completion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a helpful tutor explaining concepts in clear, simple terms."
            },
            {
              role: "user",
              content: prompt
